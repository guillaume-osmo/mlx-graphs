...
from mlx_graphs.nn.conv import GATv2Conv, GATv2Regressor
...
def test_gatv2_regressor():
    regressor = GATv2Regressor(
        node_dim=8,
        edge_dim=4,
        hidden_dim=24,
        heads=3,
    ...
    assert y_hat.shape == (3, 1), "GATv2Regressor output shape failed"
    assert mx.all(mx.isfinite(y_hat)).item(), "GATv2Regressor output should be finite"
